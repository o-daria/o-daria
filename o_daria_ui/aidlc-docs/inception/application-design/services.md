...
useQuery(
  ['report', projectId],
  () => reportsApi.getReportData(projectId),
...
const generateMutation = useMutation(async (projectId: string) => {
  ...
  const { canvaLink }    = await canvaApi.canvaGenerate({ projectId, sessionToken });
  return canvaLink;
});
```
  ...
  useEffect(() => {
    const source = createProjectStatusSSE({
      ...
      onStatusChange: (newStatus) => {
        setStatus(newStatus);
        if (newStatus === 'REPORT_READY' || newStatus === 'PRESENTATION_READY') {
