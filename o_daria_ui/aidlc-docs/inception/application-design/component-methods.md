...
class ProjectsApiService {
  ...
  updateProject(projectId: string, input: Partial<ProjectInput>): Promise<Project>;
...
```typescript
interface CanvaSetupRequest  { projectId: string; }
interface CanvaSetupResponse { sessionToken: string; } // or TBD with backend team

interface CanvaGenerateRequest  { projectId: string; sessionToken: string; }
interface CanvaGenerateResponse { canvaLink: string; }
...
function useReportData(projectId: string, enabled: boolean): {
