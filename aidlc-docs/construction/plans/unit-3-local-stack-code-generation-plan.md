...
- **Application code locations**: Monorepo root + `o_daria_ui/` (NEVER aidlc-docs/)
...
**Content**: 4 services — db (pgvector:pg16), ollama, api (built from o_daria_be/), nginx (built from o_daria_ui/Dockerfile.local)
- db: same SQL init files as o_daria_be/docker-compose.yml (all 3 schema files)
- api: `env_file: ./o_daria_be/.env` + runtime overrides incl. `FRONTEND_URL: http://localhost:8080`
- nginx: build args for `GOOGLE_CLIENT_ID` + all `VITE_MFE_*_URL` as same-host paths
- Port: `8080:80`
**Traceability**: FR-03, NFR-04

### Step 2: `o_daria_ui/Dockerfile.local` — multi-stage FE build [x]
**Action**: CREATE `o_daria_ui/Dockerfile.local`
...
### Step 3: `o_daria_ui/infra/nginx/nginx.local.conf` — nginx routing [x]
**Action**: CREATE `o_daria_ui/infra/nginx/nginx.local.conf`
**Content**: 
