...
| `infra/terraform/modules/s3-hosting/main.tf` | FE S3 bucket (migrated from `o_daria_ui/`). OAC resource moved here (single ownership). Bucket policy uses `cloudfront_distribution_arn` input |
...
| File | Change |
|------|--------|
| `o_daria_ui/.github/workflows/deploy.yml` | Added `GOOGLE_CLIENT_ID: ${{ secrets.GOOGLE_CLIENT_ID }}` to build env — bakes client ID into webpack bundles at CI build time |
