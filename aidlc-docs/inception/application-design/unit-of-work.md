...
**Type**: Module within existing `o_daria_be` service
**Owner**: Backend
...
| File | Purpose |
|------|---------|
| `o_daria_be/src/db/schema_auth.sql` | `users` + `sessions` tables |
| `o_daria_be/src/routes/auth.routes.js` | `POST /auth/google` route handler |
| `o_daria_be/src/services/googleAuthService.js` | `verifyGoogleToken`, `upsertTenant`, `upsertUser`, `createSession` |
| `o_daria_be/src/middleware/auth.middleware.js` | `authenticate()` — DB session lookup + API_KEY shortcut |
| `o_daria_be/src/services/s3Service.js` | `uploadToS3`, `getS3Key` |

### Files to Modify
| File | Change |
|------|--------|
| `o_daria_be/src/app.js` | Mount `AuthRouter`; import `AuthMiddleware`; add `FRONTEND_URL` to CORS; wire `S3Service` into `POST /reports` |
| `o_daria_be/scripts/migrate.sh` | Add `schema_auth.sql` as 3rd file |
| `o_daria_be/docker-compose.yml` | Mount `schema_auth.sql` as `03_schema_auth.sql` in db init |
| `o_daria_be/package.json` | Add `google-auth-library`, `@aws-sdk/client-s3` |
| `o_daria_be/.env.example` | Add `GOOGLE_CLIENT_ID`, `FRONTEND_URL`, `S3_IMAGES_BUCKET`, `AWS_REGION`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY` |
| `o_daria_be/Dockerfile` | Pin `node:20-alpine` to specific digest (NFR-06) |
...
**Type**: Modules within existing `o_daria_ui` workspace
**Owner**: Frontend
...
| File | Purpose |
|------|---------|
| `o_daria_ui/packages/@app/auth/src/googleAuthService.ts` | `loginWithGoogleCredential(credential)` — POST to BE |

### Files to Modify
| File | Change |
|------|--------|
| `o_daria_ui/packages/@app/auth/src/tokenStorage.ts` | Add `getToken`, `setToken`, `clearAll` |
| `o_daria_ui/packages/@app/auth/src/types.ts` | Add `name` to `User`; add `GoogleAuthResponse`; soft-deprecate old types |
| `o_daria_ui/packages/@app/auth/src/AuthService.ts` | Add `loginWithGoogle`; soft-deprecate email/password methods |
| `o_daria_ui/packages/@app/auth/src/AuthContext.tsx` | Replace `login` with `loginWithGoogle` |
| `o_daria_ui/packages/@app/auth/src/AuthProvider.tsx` | Token injection; session restore on mount; 401 handler wiring |
| `o_daria_ui/packages/@app/auth/src/index.ts` | Export new types + `loginWithGoogle` |
| `o_daria_ui/apps/mfe-auth/src/pages/LoginPage.tsx` | Replace Formik form with `<GoogleLogin>` |
| `o_daria_ui/apps/mfe-auth/package.json` | Add `@react-oauth/google` |
| `o_daria_ui/apps/mfe-auth/webpack.config.js` | Add `GOOGLE_CLIENT_ID` to `DefinePlugin` |
| `o_daria_ui/apps/shell/src/App.tsx` | Add `GoogleOAuthProvider` wrapper |
| `o_daria_ui/apps/shell/webpack.config.js` | Add `GOOGLE_CLIENT_ID` to `DefinePlugin` |
| `o_daria_ui/.env.example` | Add `GOOGLE_CLIENT_ID` |

### Files to Remove Routes From (Q7=B — routes removed, files kept)
| File | Change |
|------|--------|
| `o_daria_ui/apps/mfe-auth/src/` router/config | Remove routes for Register, ForgotPassword, ResetPassword pages |
...
| `o_daria_ui/Dockerfile.local` | Multi-stage: pnpm build → nginx static serve |
| `o_daria_ui/infra/nginx/nginx.local.conf` | MFE path routing + `/api/` proxy + security headers |
...
| `o_daria/infra/terraform/modules/s3-hosting/` | Migrated from `o_daria_ui/infra/terraform/modules/s3-hosting/` |
...
| File | Change |
|------|--------|
| `o_daria_ui/.github/workflows/deploy.yml` | Add `GOOGLE_CLIENT_ID: ${{ secrets.GOOGLE_CLIENT_ID }}` to build env |
