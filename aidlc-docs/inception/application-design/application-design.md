...
| ------------ | ------------------------------------------------------- | ---------------------------------------------------------- |
| FE Terraform | `ui/infra/terraform/` — FE only (1 CF + 1 S3)           | Migrate to `o_daria/infra/terraform/` — unified full-stack |
...
| ------------------- | --------------------------------------- | ----------------------------------------- |
| `AuthRouter`        | `api/src/routes/auth.routes.js`         | `POST /auth/google` endpoint              |
| `GoogleAuthService` | `api/src/services/googleAuthService.js` | Token verify, user upsert, session create |
| `AuthMiddleware`    | `api/src/middleware/auth.middleware.js` | Session DB lookup + API_KEY dev shortcut  |
| `S3Service`         | `api/src/services/s3Service.js`         | Image upload to S3 images bucket          |

### Backend — Modified

| Component            | File                     | Change                                                                                               |
| -------------------- | ------------------------ | ---------------------------------------------------------------------------------------------------- |
| `app.js`             | `api/src/app.js`         | Mount AuthRouter; import AuthMiddleware; add FRONTEND_URL to CORS; wire S3Service into POST /reports |
| `migrate.sh`         | `api/scripts/migrate.sh` | Add `schema_auth.sql` as 3rd file                                                                    |
| `docker-compose.yml` | `api/docker-compose.yml` | Mount `schema_auth.sql` as `03_schema_auth.sql` in db init                                           |

### Frontend — New

| Component              | File                                             | Purpose                          |
| ---------------------- | ------------------------------------------------ | -------------------------------- |
| `googleAuthService.ts` | `ui/packages/@app/auth/src/googleAuthService.ts` | HTTP call to `POST /auth/google` |
...
| `Dockerfile.local`         | `ui/Dockerfile.local`                        | Multi-stage: pnpm build → nginx static serve      |
| `nginx.local.conf`         | `ui/infra/nginx/nginx.local.conf`            | MFE path routing + API proxy + security headers   |

### Infrastructure — Migrated/Modified

| Component             | From                  | To                           | Change                                                                    |
| --------------------- | --------------------- | ---------------------------- | ------------------------------------------------------------------------- |
| `modules/s3-hosting/` | `ui/infra/terraform/` | `o_daria/infra/terraform/`   | Migrate unchanged                                                         |
| `modules/cloudfront/` | `ui/infra/terraform/` | `o_daria/infra/terraform/`   | Update CSP for Google Sign-In (`accounts.google.com`, `*.googleapis.com`) |
| `modules/iam-deploy/` | `ui/infra/terraform/` | `o_daria/infra/terraform/`   | Migrate unchanged                                                         |
| `schema_auth.sql`     | (new)                 | `api/src/db/schema_auth.sql` | New — users + sessions tables                                             |
...
6. **Terraform migration**: Existing `ui/infra/terraform/` modules are migrated to `o_daria/infra/terraform/`. State backend reuses the existing `o-daria-ui-tfstate` S3 bucket under a new key. The old FE-only Terraform directory is left in place (not deleted) until the new root Terraform is verified.
